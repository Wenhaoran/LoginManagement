...
<script type="text/javascript">
    ...
    $(function() {
        ...
        detailDataGrid = $('#detailDataGrid').datagrid({
            url : '${path}/pages/account/dataGridByModuleId?moduleId='+moduleId,
            striped : true,
            rownumbers : true,
            pagination : true,
            singleSelect : false,
    ...
    function addAMRSFun() {
    	var moduleId = '${module.id}';
    	var url="${path}/pages/account/moduleGiveAccount?moduleId="+moduleId;//?invoiceId="+'${purchInvoice.id}'+"&billType=1
...
//     	$('#idsDetail').val(ids);
        progressLoad();
        var moduleId = '${module.id}';
        $.post('${path}/pages/account/deleteAMRS', {
            ids : ids,
            moduleId : moduleId
        }, function(result) {
            if (result.success) {
...
<body>	
    <div class="easyui-layout" data-options="fit:true,border:true,title:'进货发票明细列表'">
			<div data-options="region:'north',fit:false,border:false,title:'查询条件',hideCollapsedContent:false" style="height:70px;overflow: hidden;padding:1px;background-color: #fff">
	    		<form id="detailSearchForm">
	    			<input id='"moduleId"' name="moduleId" value='${module.id}' type="hidden" />
	    			<input type="hidden" id="idsDetail" name="ids" value="" />
	    			<div class="search_condition">
	    				<p>
							<b>当前模块：</b>
	                		<b style="color: red;">${module.name}</b>
						</p>
    					<p>
							<b>账户名称</b>
	                		<input id="module_query" name="name" type="text" placeholder="请输入账户名称" class="easyui-validatebox span2" maxlength="32" value=""/>
						</p>
    					<p>
							<b>账户类型</b>
    ...
    <div id="detailToolbar" style="display: none;">
        	<a onclick="addAMRSFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">关联账户</a>
