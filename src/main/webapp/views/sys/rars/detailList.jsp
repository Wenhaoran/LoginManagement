...
<script type="text/javascript">
    ...
    $(function() {
        ...
        detailDataGrid = $('#detailDataGrid').datagrid({
            url : '${path}/pages/role/dataGridByAccountId?accountId='+accountId,
            striped : true,
            rownumbers : true,
            pagination : true,
            singleSelect : false,
    ...
    function addRARSFun() {
    	var accountId = '${account.id}';
    	var url="${path}/pages/role/accountGiveRole?accountId="+accountId;//?invoiceId="+'${purchInvoice.id}'+"&billType=1
...
//     	$('#idsDetail').val(ids);
        ...
        $.post('${path}/pages/role/deleteRARS', {
            ids : ids,
...
<body>	
    <div class="easyui-layout" data-options="fit:true,border:true,title:'进货发票明细列表'">
			<div data-options="region:'north',fit:false,border:false,title:'查询条件',hideCollapsedContent:false" style="height:70px;overflow: hidden;padding:1px;background-color: #fff">
	    		<form id="detailSearchForm">
	    			<input id='accountId' name="accountId" value='${account.id}' type="hidden" />
	    			<input type="hidden" id="idsDetail" name="ids" value="" />
	    			<div class="search_condition">
	    				<p>
							<b>当前账户：</b>
	                		<b style="color: red;">${account.name}</b>
						</p>
    					<p>
							<b>角色名称</b>
    ...
    <div id="detailToolbar" style="display: none;">
        	<a onclick="addRARSFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">关联角色</a>
