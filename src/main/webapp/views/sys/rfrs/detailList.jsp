...
<script type="text/javascript">
    ...
    $(function() {
        ...
        detailDataGrid = $('#detailDataGrid').datagrid({
            url : '${path}/pages/role/dataGridByFunctionId?functionId='+functionId,
            striped : true,
            rownumbers : true,
            pagination : true,
            singleSelect : false,
    ...
    function addRFRSFun() {
    	var functionId = '${function.id}';
    	var url="${path}/pages/role/functionGiveRole?functionId="+functionId;
    	
        openEditDialog(url, 500, 600, detailDataGrid, function(cid) {
			addDetail(cid);
		}).dialog('open').dialog('setTitle', '添加角色关联');
    }
    ...
    function detailDeleteFun() {
    	...
    	$('#idsDetail').val(ids);
        progressLoad();
        $.post('${path}/pages/role/deleteRFRS', {
            ids : ids,
...
<body>	
    <div class="easyui-layout" data-options="fit:true,border:true,title:'功能关联角色列表'">
			<div data-options="region:'north',fit:false,border:false,title:'查询条件',hideCollapsedContent:false" style="height:70px;overflow: hidden;padding:1px;background-color: #fff">
	    		<form id="detailSearchForm">
	    			<input id='functionId' name="functionId" value='${function.id}' type="hidden" />
	    			<input type="hidden" id="idsDetail" name="ids" value="" />
	    			<div class="search_condition">
	    				<p>
							<b>当前功能：</b>
	                		<b style="color: red;">${function.name}</b>
						</p>
    					<p>
							<b>角色名称</b>
    ...
    <div id="detailToolbar" style="display: none;">
        	<a onclick="addRFRSFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">关联角色</a>
